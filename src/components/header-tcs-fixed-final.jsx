...
export default function HeaderTCSFixedFinal() {
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false)
    const [mobileActiveTab, setMobileActiveTab] = useState(null)
    const [scrolled, setScrolled] = useState(false)
    const [searchOpen, setSearchOpen] = useState(false)

    // Navigation items based on actual project structure
    const mainNavItems = [
        {
            name: "Services",
            url: "/services",
            ...
            dropdownSections: [
                ...
                },
                {
                    title: "Solutions",
                    links: [
                        { name: "Solutions Overview", url: "/solution" },
            ...
            dropdownSections: [
                ...
                },
                {
                    ...
                    links: [
                        ...
                        { name: "Partner Details", url: "/partner" },
                    ]
                }
            ]
        },
    ...
    const toggleMobileTab = (tabName) => {
        setMobileActiveTab(mobileActiveTab === tabName ? null : tabName);
    };
    ...
    return (
        <>
            ...
            {/* Main Header - TCS Style with Hover */}
            <header className={`sticky top-0 z-40 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-2' : 'bg-white py-3'}`}>
                <div className="container mx-auto px-4">
                                    ...
                                    <li key={index} className="nav-item">
                                        ...
                                        >
                                            ...
                                            {item.hasDropdown && (
                                                <ChevronDown 
                                                    size={16} 
                                                    className="ml-1 nav-arrow transition-transform duration-300" 
                                                />
                                            )}
                                        </Link>
                                        
                                        {/* TCS-style mega dropdown on hover */}
                                        {item.hasDropdown && (
                                                            ...
                                                            <ul className="space-y-2">
                                                                {section.links.map((link, linkIndex) => (
                                                                    <li key={linkIndex}>
                                                                        ...
                                                                        >
                                                                            <ChevronRight size={14} className="mr-1 opacity-0 hover:opacity-100 transition-opacity" />
                                                                            {link.name}
                                                                        </Link>
                                                                    </li>
                                                                ))}
                                ...
                                <li key={index} className="mb-2">
                                    <div className="flex items-center justify-between">
                                        <Link 
                                            ...
                                            onClick={item.hasDropdown ? undefined : toggleMobileMenu}
                                        >
                                            {item.name}
                                        </Link>
                                        ...
                                        {item.hasDropdown && (
                                            ...
                                            >
                                                <ChevronDown 
                                                    size={20} 
                                                    className={`transition-transform duration-300 ${mobileActiveTab === item.name ? 'rotate-180' : ''}`} 
                                                />
                                            </button>
                                        )}
                                    </div>
                                    ...
                                    {item.hasDropdown && (
                                        <div className={`overflow-hidden transition-all duration-300 ${mobileActiveTab === item.name ? 'max-h-[500px]' : 'max-h-0'}`}>
