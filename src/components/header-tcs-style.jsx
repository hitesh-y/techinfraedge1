...
import { Phone, Mail, Menu, X, ChevronDown, Search, Globe, Clock, User } from "lucide-react"
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center space-x-4">
                            {siteData.socialLinks.map((social, index) => (
                                <a key={index} href={social.url} className="hover:text-gray-200 transition-colors hover:scale-110 duration-300">
                ...
                <div className="container mx-auto px-4">
                    ...
                    <div className="flex items-center justify-between py-3 border-b">
                        <div className="flex items-center">
                            <Link href="/" className="logo mr-8 transition-transform duration-300 hover:scale-105">
                                <Image 
                                    ...
                                    style={{ height: scrolled ? '35px' : '40px', width: 'auto' }} 
                                />
                            </Link>
                        </div>
                    ...
                    <div className="hidden md:block py-3">
                        <nav className="flex justify-between">
                            <ul className="flex space-x-8">
                                {siteData.navLinks.map((link, index) => (
                                    <li key={index} className="relative group">
                                        <Link 
                                            href={link.url} 
                                            className="flex items-center py-2 text-gray-800 hover:text-primary transition-colors font-medium after:content-[''] after:absolute after:h-0.5 after:w-0 after:left-0 after:bottom-0 after:bg-primary group-hover:after:w-full after:transition-all after:duration-300"
                                        >
                                            {link.name}
                                            {link.megaMenu && (
                                                    ...
                                                    <ul className="grid grid-cols-2 gap-2 min-w-[320px]">
                                                        {link.subMenu.map((subLink, subIndex) => (
                                                            <li key={subIndex}>
                                                                <Link 
                                                                    href={subLink.url} 
                                                                    className="block px-4 py-2 hover:bg-gray-50 rounded-md text-gray-700 hover:text-primary transition-colors hover:translate-x-1 duration-300"
                                                                >
                                                                    {subLink.name}
                                                                </Link>
                                                            </li>
                            ...
                            <div>
                                <Link 
                                    href="/contact" 
                                    className="inline-flex items-center px-6 py-2 bg-primary text-white rounded-md hover:bg-primary/90 transition-all duration-300 hover:shadow-lg transform hover:-translate-y-1"
                                >
                                    Get in Touch
                                </Link>
                            </div>
                                            ...
                                            <ul className="pl-4 py-2 space-y-2">
                                                {link.subMenu.map((subLink, subIndex) => (
                                                    <li key={subIndex}>
                                                        <Link 
                                                            href={subLink.url} 
                                                            className="py-2 block text-gray-600 hover:text-primary transition-all duration-300 hover:translate-x-2"
                                                            onClick={toggleMobileMenu}
                                                        >
                                                            {subLink.name}
                                                        </Link>
